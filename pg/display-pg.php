...
$validatorQuery = "SELECT DISTINCT nama_validator FROM pg ORDER BY nama_validator";
$validatorResult = $conn->query($validatorQuery);

$instansiQuery = "SELECT DISTINCT instansi_kerja FROM pg ORDER BY instansi_kerja";
$instansiResult = $conn->query($instansiQuery);

$statusUsulanQuery = "SELECT DISTINCT status_usulan FROM pg ORDER BY status_usulan";
$statusUsulanResult = $conn->query($statusUsulanQuery);

$tahunQuery = "SELECT DISTINCT YEAR(tanggal_usulan) AS tahun FROM pg ORDER BY tahun";
$tahunResult = $conn->query($tahunQuery);

$bulanQuery = "SELECT DISTINCT MONTH(tanggal_usulan) AS bulan FROM pg ORDER BY bulan";
...
$totalDataQuery = "SELECT COUNT(*) AS total_data FROM pg WHERE 1=1";
if ($filterValidator) {
...
$instansiTotalQuery = "SELECT instansi_kerja, COUNT(*) AS total_verifikasi FROM pg WHERE 1=1";
if ($filterStatusUsulan) {
...
$instansiValidatorQuery = "SELECT instansi_kerja, COUNT(*) AS total_verifikasi_per_validator FROM pg WHERE 1=1";
if ($filterValidator) {
...
if (isset($_GET['action']) && $_GET['action'] == 'getInstansiDetails') {
    ...
    $detailsQuery = "SELECT nama, nip, alasan_tolak FROM pg WHERE 1=1";
...
if (isset($_POST['delete_all'])) {
    ...
    if ($confirm) {
        ...
        if ($conn->query($deleteAllQuery) === TRUE) {
            ...
            $instansiTotalResult = $conn->query("SELECT instansi_kerja, COUNT(*) as total_verifikasi FROM pg GROUP BY instansi_kerja");
            $instansiValidatorResult = $conn->query("SELECT instansi_kerja, COUNT(*) as total_verifikasi_per_validator FROM pg GROUP BY instansi_kerja");
        } else {
...
<body>
    ...
    <h1>Data Verifikasi PG</h1>
    <a href="import-pg.php" class="button-link">Upload Excel</a>
